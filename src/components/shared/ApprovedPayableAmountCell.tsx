...
const ApprovedPayableAmountCell: FC<Props> = (props) => {
  const { row, id } = props;
  ...
  return (
    <DoubleTextCell
      primaryText={`${currencyFormatter(ApprovedPayableAmount, PayableAmountCurrency)}`}
      secondaryText={`Fatura Tutarı: ${currencyFormatter(PayableAmount, PayableAmountCurrency)}`}
...
export default ApprovedPayableAmountCell;
