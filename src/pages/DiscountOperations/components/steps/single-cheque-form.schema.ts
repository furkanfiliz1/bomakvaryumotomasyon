...
export const createSingleChequeSchema = (options: SchemaOptions = {}) => {
  ...
  return yup.object().shape({
    ...
    // Banka Bilgileri
    bankEftCode: fields
      .select(banksOptions, 'number', ['id', 'name'])
    ...
    bankBranchEftCode: fields
      .select(branchesOptions, 'number', ['id', 'name'])
    ...
    _: fields.customComponent(() => React.createElement(Divider, { sx: { my: 2 } })),
    ...
    __: fields.customComponent(() => React.createElement(Divider, { sx: { my: 2 } })),
    ...
    ___: fields.customComponent(() => React.createElement(Divider, { sx: { my: 2 } })),
    ...
    // File upload fields (managed by Dropzone components)
    frontImageFile: yup.mixed().nullable().meta({ field: SchemaField.InputHidden, visible: false }),
    backImageFile: yup.mixed().nullable().meta({ field: SchemaField.InputHidden, visible: false }),
    invoiceFile: yup.mixed().nullable().meta({ field: SchemaField.InputHidden, visible: false }),
