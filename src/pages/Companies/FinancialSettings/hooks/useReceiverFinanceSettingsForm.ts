...
interface ReceiverFinanceSettingsFormData {
  Currencies: number[];
...
interface UseReceiverFinanceSettingsFormProps {
  initialData?: Partial<FinancerRatioAFModel>;
  initialCurrencies?: number[];
}

/**
 * Custom hook for Receiver Finance Settings form management
 * Following OperationPricing patterns with Form component integration
 */
export const useReceiverFinanceSettingsForm = ({
  initialData,
  initialCurrencies = [],
}: UseReceiverFinanceSettingsFormProps) => {
  ...
  // Initial form values
  const initialValues: ReceiverFinanceSettingsFormData = useMemo(
    () => ({
  ...
  const schema = useMemo(
    () =>
      yup.object({
        ...
        IsIbanRequired: fields.switchField.label('IBAN girişi zorunlu mu?').meta({ col: 12 }),
  ...
  const form = useForm<ReceiverFinanceSettingsFormData>({
