...
interface SpotLoanWithoutInvoiceCurrencyAndSettingsFormData {
  Currencies: number[];
  IsAutoReturned: boolean;
  IsManuelCharged: boolean;
}

interface UseSpotLoanWithoutInvoiceCurrencyAndSettingsFormProps {
  initialData?: Partial<FinancerRatioSpotWithoutInvoiceModel>;
  initialCurrencies?: number[];
}

/**
 * Custom hook for Spot Loan Without Invoice Currency and Settings form management
 * Following OperationPricing patterns with Form component integration
 */
export const useSpotLoanWithoutInvoiceCurrencyAndSettingsForm = ({
  initialData,
  initialCurrencies = [],
}: UseSpotLoanWithoutInvoiceCurrencyAndSettingsFormProps) => {
  ...
  // Initial form values
  const initialValues: SpotLoanWithoutInvoiceCurrencyAndSettingsFormData = useMemo(
    () => ({
  ...
  const form = useForm<SpotLoanWithoutInvoiceCurrencyAndSettingsFormData>({
