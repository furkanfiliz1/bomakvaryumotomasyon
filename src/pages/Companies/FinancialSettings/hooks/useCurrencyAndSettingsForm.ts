...
interface CurrencyAndSettingsFormData {
  Currencies: number[];
  IsAutoReturned: boolean;
  IsManuelCharged: boolean;
}

interface UseCurrencyAndSettingsFormProps {
  initialData?: Partial<FinancerRatioModel>;
  initialCurrencies?: number[];
}

/**
 * Custom hook for Currency and Settings form management
 * Following OperationPricing patterns with Form component integration
 */
export const useCurrencyAndSettingsForm = ({
  initialData,
  initialCurrencies = [],
}: UseCurrencyAndSettingsFormProps) => {
  ...
  // Initial form values
  const initialValues: CurrencyAndSettingsFormData = useMemo(
    () => ({
  ...
  const form = useForm<CurrencyAndSettingsFormData>({
